...
(async function main() {
    try {
        ...
        const conn1Helper = new ConnectionHelper(
            conn1,
            ConfigMap.MYSQL_DATABASE_1
        );
        const conn2Helper = new ConnectionHelper(
            conn2,
            ConfigMap.MYSQL_DATABASE_2
        );
        ...
        const compareStructRes = await conn2Helper.compareStructure(
            conn1Helper,
            'email_trigger_has_process'
        );
